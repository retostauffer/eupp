...
```{r gridded_forecast_hr_get_gridded, include = FALSE}
# Same eupp_get_gridded call as above but this one is executed
# as we need the object in the next section.
# Note: Allows for one single date only!
conf$date <- conf$date[1]
...
```{r}
library("ggplot2")
class(data) <- class(data)[-1] # Dropping eupp_stars class attribute
ggplot() + geom_stars(data = data["t2m"])
```

@TODO: `ggplot() + geom_stars(data = data["q"])` fails when `data` is an object
of class `c("eupp_stars", "stars")`. Unfortunately `geom_stars` is not an S3 method;
currently there is no other way than dropping the `eupp_stars` class attribute.
...
(interpolated <- st_extract(data, locations, bilinear = FALSE, atname = "name"))

library("ggplot2")
class(data) <- class(data)[-1]
ggplot() + geom_stars(data = data["q"])
```
...
library("ggplot2")
class(data) <- class(data)[-1]
ggplot() + geom_stars(data = data["t2i"])
```
...
(test <- st_extract(data, locations, bilinear = TRUE))
```
...
```{r gridded_forecast_ens_processing, out.width = "100%", fig.width = 10, fig.height = 6}
library("sf")
...
library("ggplot2")
class(data) <- class(data)[-1]
ggplot() + geom_stars(data = data["cp", , , 1:4])
```
