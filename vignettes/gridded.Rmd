...
conf <- eupp_config(type = "forecast",
                    ...
                    date  = "2017-01-02",
                    parameter = c("cp", "2t"),
                    steps  = c(24L, 120L, 240L), # +1, +5, +10 days ahead
...
```{r}
eupp_download_gridded(conf, output_file = "_test.grb", output_format = "grb", overwrite = TRUE)
```
...
```{r demoplot_stars, fig = TRUE, fig.width = 10, fig.height = 5, out.width = "60%"}
library("stars")
...
plot(x["t2m"],
     ...
     breaks = seq(min(x$t2m), max(x$t2m), length = 22))

# Plotting convective precipitation (mm)
plot(x["cp"] * 1e3,
     main   = "cp",
     col    = hcl.colors(10, "Purple-Yellow", rev = TRUE),
     breaks = 1e3 * seq(0, as.numeric(max(x$cp)), length = 11))
```
...
```{r demoplot_ggplot, fig = TRUE, fig.width = 10, fig.height = 7, out.width = "60%"}
library("sf")
library("ggplot2")
library("colorspace")
